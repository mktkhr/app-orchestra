...
eval-shortlist: build ## Measure the product's own planner on the narrowing corpus, narrowing on and off, or per wording with WORDING=a,b,c, THINKING=on|off, REPEAT_PENALTY=<float> and STAGES=1|2 (docs/specs/shortlisting.md, docs/plans/wording.md, docs/plans/staging.md; never part of make check; needs llama-swap running the models named in e2e/shortlist/run.ts); PICKER=jev for the Jev trial or PICKER=hybrid for the hybrid Jev/local picker, ORCHESTRA_JEV_API_KEY from the caller's own environment, JEV_CRITERIA=v2 for its second round, GATE=jev for its v3 noul refusal gate
