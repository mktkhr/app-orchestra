...
| **H8** | The planner's own thinking defaults off, with a per-question override. Measured 2026-09-16 against the real planner: thinking off gives **correct@1 67 / correct@shown 71** at a mean **1377ms**, and never hits `max_tokens`; thinking on gives **67 / 70** at a mean **6223ms**. Off wins on both correctness and latency, so `ORCHESTRA_PLANNER_THINKING` now defaults to `off` (previously `on`). A person can still ask one question with thinking on: `PlanRequest.thinking` (optional `boolean`) overrides the platform's default for that one call, and the browser shows it as a switch beside the question form, labelled exactly「思考」- nothing else, no tooltip prose.                                                                    |
