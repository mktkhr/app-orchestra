...
| **S3** | The pick offers three fixed lines after the shortlist, so the built-ins keep a way in: `list_capabilities` (使える操作の一覧そのものを求めている), `propose_panel` (画面に出したい) and `none` (どの候補も質問に合わない（業務と無関係な質問、候補の操作では答えられない質問）) - the first and third reworded 2026-09-18, section 7's second and third allowed attempts, after `propose_panel` became conditional. `propose_panel` is offered only when the request carries a workspace, the same condition `usecase.ToolsFor` applies (`docs/specs/offering.md`), so a workspace-less pick sees two fixed lines, not three. The pick is the only place the fixed _line_ is offered; whether a proposal actually results is the fill's own decision (section 5), not this line's - the pick's `propose_panel` line still exists only for a question that names no concrete operation at all (S1's own single-call fallback), because the pick almost always names a real operation first (section 1's regression). Their wording is measured by `make eval`'s `capability` and `unanswerable` cases, not guessed; `propose_panel` has no eval case and is covered by the orchestrator's own tests. |
