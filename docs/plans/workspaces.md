...
- [ ] **Step 1** Write the store's test first: create a workspace, add two
      ...
      expect failure.
- [ ] **Step 2** Add `modernc.org/sqlite` to `services/platform/go.mod`. Write
      the schema as an embedded statement applied at open. Implement the store
      until the test passes.
- [ ] **Step 3** Add `ORCHESTRA_DB_PATH` to config, with a test. Wire the store
...
- [ ] **Step 5** Commit: `feat(platform): keep workspaces in a file`
...
- [ ] **Step 1** Add the paths and schemas. `make api-lint`, then
      `make generate`.
- [ ] **Step 2** Write the acceptance test: create a workspace, add a panel,
...
- [ ] **Step 3** Implement the handlers until it passes.
- [ ] **Step 4** `make -k check` — nothing failing but what the frontend has
      not caught up with.
- [ ] **Step 5** Commit: `feat(platform): serve workspaces and their panels`
...
- [ ] **Step 1** Write the test: with a scripted API returning two workspaces,
      both names appear in the drawer; creating one posts and appends it. Run it,
      expect failure.
- [ ] **Step 2** Implement. Deleting asks first — it takes the panels with it.
- [ ] **Step 3** `make web-lint web-test guard-fsd guard-ui guard-duplication`
      green.
- [ ] **Step 4** Commit: `feat(web): list workspaces in the drawer`
...
- [ ] **Step 1** Write the test: a workspace of two panels, one whose invoke
...
- [ ] **Step 2** Implement. The panels load independently; one slow service does
      not hold the others.
- [ ] **Step 3** `make web-lint web-test guard-fsd guard-ui guard-duplication`
      green, then `make build` and `make guard-browser`.
- [ ] **Step 4** Commit: `feat(web): draw a workspace's panels`
...
- [ ] **Step 1** Write the test: a table result offers the control; using it
      posts a panel whose service, operation id and args match the result's
      provenance. Run it, expect failure.
- [ ] **Step 2** Implement. Which workspace to save to is chosen from the ones
      that exist; if there are none, the control offers to make one.
- [ ] **Step 3** Web gates green.
- [ ] **Step 4** Commit: `feat(web): save a result to a workspace`
...
- [ ] **Step 1** Write the test: a panel whose second invoke returns a row the
      first did not; refreshing shows the new row. Run it, expect failure.
- [ ] **Step 2** Implement. The control does not disable itself while in flight
      — it shows that it is working (`make guard-layout`).
- [ ] **Step 3** Web gates green.
- [ ] **Step 4** Commit: `feat(web): run a panel again`
...
- [ ] **Step 1** Write the test: asking on a workspace screen appends a turn,
      and the result's save control defaults to that workspace. Run it, expect
      failure.
- [ ] **Step 2** Implement. If the conversation feature needs a prop to know
      which workspace it sits on, that is the whole change.
- [ ] **Step 3** Web gates green, then `make build` and `make guard-browser`.
- [ ] **Step 4** Commit: `feat(web): ask a question from a workspace`
