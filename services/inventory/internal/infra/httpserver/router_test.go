...
func TestNewRouterServesItems(t *testing.T) {
	router, err := httpserver.NewRouter(handler.NewItems(repository.NewMemory()))
	require.NoError(t, err)

	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/api/inventory/items", http.NoBody)
