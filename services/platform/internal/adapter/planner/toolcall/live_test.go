...
// liveTimeout bounds a call to a real, possibly slow local model.
const liveTimeout = 60 * time.Second

// TestPlanAgainstALiveModel exercises this package's whole path - real
// HTTP, a real model choosing a real tool - against whatever
// OpenAI-compatible endpoint ORCHESTRA_LLM_BASE_URL names. It never runs
// under `make check` (the global constraint, docs/plans/orchestration.md):
// it t.Skip()s unless ORCHESTRA_LIVE_LLM=1, which nothing in the harness
// sets. Run it by hand against llama-swap:
//
//	ORCHESTRA_LIVE_LLM=1 ORCHESTRA_LLM_BASE_URL=http://localhost:11435/v1 \
//	  ORCHESTRA_LLM_MODEL=qwen3.5-9b-q8 go test ./internal/adapter/planner/toolcall/... -run Live -v
func TestPlanAgainstALiveModel(t *testing.T) {
	...
	planner := toolcall.New(client, catalog)

	ctx, cancel := context.WithTimeout(context.Background(), liveTimeout)
	defer cancel()

	decision, err := planner.Plan(ctx, "検品保留の在庫を見せて", nil, nil, usecase.ToolsFor(catalog, usecase.PlanContext{WorkspaceID: "ws-1"}), nil)
