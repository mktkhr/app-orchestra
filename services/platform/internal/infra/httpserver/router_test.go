...
func TestNewRouterServesHealth(t *testing.T) {
	router, err := httpserver.NewRouter(handler.NewAPI(handler.NewHealth(), handler.NewSession(nil, nil), handler.NewPlan(nil), handler.NewInvoke(nil), handler.NewWorkspace(nil)), "", fakeSessionUsers{})
...
func TestNewRouterServesStaticDir(t *testing.T) {
	...
	router, err := httpserver.NewRouter(handler.NewAPI(handler.NewHealth(), handler.NewSession(nil, nil), handler.NewPlan(nil), handler.NewInvoke(nil), handler.NewWorkspace(nil)), dir, fakeSessionUsers{})
	require.NoError(t, err)
