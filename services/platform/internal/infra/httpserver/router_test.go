...
func TestNewRouterServesHealth(t *testing.T) {
	router, err := httpserver.NewRouter(handler.NewAPI(handler.NewHealth(), handler.NewSession(nil, nil), handler.NewPlan(nil), handler.NewInvoke(nil), handler.NewWorkspace(nil)), "", nil)
...
func TestNewRouterServesStaticDir(t *testing.T) {
	...
	router, err := httpserver.NewRouter(handler.NewAPI(handler.NewHealth(), handler.NewSession(nil, nil), handler.NewPlan(nil), handler.NewInvoke(nil), handler.NewWorkspace(nil)), dir, nil)
	require.NoError(t, err)
