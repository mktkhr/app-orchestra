...
func TestPlanListCapabilitiesWithServiceFiltersToThatService(t *testing.T) {
	...
	result, err := orchestrator.Plan(t.Context(), adminUser(), "在庫について、どういう操作ができる？", nil, nil, "", "", nil)
...
func TestPlanListCapabilitiesWithUnknownServiceReturnsNoRows(t *testing.T) {
	...
	result, err := orchestrator.Plan(t.Context(), adminUser(), "存在しないサービスについて何ができる？", nil, nil, "", "", nil)
