...
func TestPlanProposalCarriesTheModelsOwnValues(t *testing.T) {
	...
	result, err := orchestrator.Plan(t.Context(), adminUser(), "在庫をステータス別に棒グラフで置いて", nil, nil, "ws-1", "", nil)
...
func TestPlanProposalWithNoViewFillsInFromTheCatalogue(t *testing.T) {
	...
	result, err := orchestrator.Plan(t.Context(), adminUser(), "在庫をステータス別に置いて", nil, nil, "ws-1", "", nil)
...
func TestPlanProposalComponentFallsBackToRenderResultShapeWithNoChartHint(t *testing.T) {
	...
	result, err := orchestrator.Plan(t.Context(), adminUser(), "在庫の一覧を置いて", nil, nil, "ws-1", "", nil)
...
func TestPlanProposalOnAnOperationTheUserMayNotCallFails(t *testing.T) {
	...
	_, err := orchestrator.Plan(t.Context(), regularUser(), "在庫アイテムを作るパネルを置いて", nil, nil, "ws-1", "", nil)
...
func TestPlanProposalOnAnUnknownEndpointFails(t *testing.T) {
	...
	_, err := orchestrator.Plan(t.Context(), adminUser(), "存在しない操作のパネルを置いて", nil, nil, "ws-1", "", nil)
...
func TestPlanRefusesAProposalWithNoWorkspaceID(t *testing.T) {
	...
	_, err := orchestrator.Plan(t.Context(), adminUser(), "在庫の一覧を置いて", nil, nil, "", "", nil)
