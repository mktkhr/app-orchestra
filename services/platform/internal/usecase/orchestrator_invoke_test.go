...
// TestInvokeStripsTheSyntheticAllArgumentBeforeValidatingOrCalling pins
// D15 (docs/specs/orchestration.md, section 8a) on Invoke's own path,
// separate from Plan's call path (see
// TestPlanCallStripsTheSyntheticAllArgumentBeforeInvoking,
// orchestrator_test.go): a workspace panel replaying a saved call, or a
// person's own button press, must not be able to hand the synthetic value
// to a service either. Stripping also has to happen before validateArgs
// runs - __all__ is never a declared value of "status", so without
// stripping first this call would fail validation instead of succeeding
// with every row.
func TestInvokeStripsTheSyntheticAllArgumentBeforeValidatingOrCalling(t *testing.T) {
	invoker := &fakeInvoker{data: map[string]any{"items": []any{}}}
	orchestrator := usecase.NewOrchestrator(invokeCatalog(), &fakePlanner{}, invoker, &fakePermissionStore{})

	result, err := orchestrator.Invoke(t.Context(), adminUser(), "inventory", "ListInventoryItemsByStatus",
		map[string]any{"status": domain.EnumAllValue})
...
// TestInvokeStripsTheSyntheticAllArgumentButKeepsEveryOther mirrors the
// Plan-path test of the same name: stripping removes only the synthetic
// argument.
func TestInvokeStripsTheSyntheticAllArgumentButKeepsEveryOther(t *testing.T) {
	c := invokeCatalog()
	for i := range c.Endpoints {
		if c.Endpoints[i].OperationID == "ListInventoryItemsByStatus" {
			c.Endpoints[i].Parameters = append(c.Endpoints[i].Parameters,
				domain.Parameter{Name: "q", In: "query", Schema: domain.Schema{Type: domain.SchemaTypeString}})
		}
	}

	invoker := &fakeInvoker{data: map[string]any{"items": []any{}}}
	orchestrator := usecase.NewOrchestrator(c, &fakePlanner{}, invoker, &fakePermissionStore{})

	result, err := orchestrator.Invoke(t.Context(), adminUser(), "inventory", "ListInventoryItemsByStatus",
		map[string]any{"status": domain.EnumAllValue, "q": "棚"})
