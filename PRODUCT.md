...
| **D17** | The model that chooses an operation from a shortlist is local. A frontier model is not adopted: on the same 100 questions and the same 50 candidates, a local 9B model scores 78 and Opus 5 scores 84 - six points for two to six times the latency and a per-call cost. Thinking adds nothing on either. Measured 2026-09-15, re-scored against the corrected answer key the same day, `DECISIONS.md`. |
